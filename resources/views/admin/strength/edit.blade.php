...
<section class="page_area">
	<div class="panel">
		...
		<div class="panel_body">
			<form class="form-horizontal" action="{{route('admin.strength.update',$data->id)}}" method="POST" enctype="multipart/form-data">
				@csrf
				<div class="form-group row">
					<label for="inputEmail3" class="col-sm-3 col-form-label text-right">Icon :<span>*</span></label>
					<div class="col-sm-6">
						<input type="text" value="{{$data->icon}}" class="form-control @error('title') is-invalid @enderror" name="icon">
						@error('icon')
				...
				<div class="form-group row">
					...
					<div class="col-sm-6">
						<input type="text" value="{{$data->stitle}}" name="stitle" class="form-control"/>
						@error('stitle')
						<div class="alert alert-danger">{{ $message }}</div>
						@enderror
					</div>
